...
- [🚀 启动](#-启动)
- [🐳 Docker / Dokploy 部署](#-docker--dokploy-部署)
- [🔧 开机自启动](#-开机自启动)
...
本 bridge 是纯 Node.js，**零第三方依赖**，所以镜像极小、无需构建步骤。它是**无状态**的——OpenCode Go key 仍由 Claude Code 逐请求传入，因此**容器内不存放任何密钥**。

> [!IMPORTANT]
> 在容器里必须绑定 `0.0.0.0`，而不是默认的 `127.0.0.1`。镜像已经替你设好了 `CLAUDE_OPENCODE_PROXY_HOST=0.0.0.0`。
...
然后把 Claude Code 指向 `http://<host>:8787`（见 [Claude Code 设置](#%EF%B8%8F-claude-code-设置)）。

### docker compose

仓库内附带了可直接使用的 [`docker-compose.yml`](./docker-compose.yml)（带命名卷持久化 reasoning 缓存，健康检查已内置在镜像中）：
...
1. 新建一个 **Compose** 应用，指向本仓库（或直接粘贴 `docker-compose.yml` 的内容）。
2. 部署。Dokploy 会拉取 `ghcr.io/cadl/...:latest` 并启动 `bridge` 服务。
3. 给 `bridge` 服务在端口 **8787** 上绑定一个**域名**——Dokploy 内置的 Traefik 代理会负责 TLS 终止与路由，因此无需映射宿主机端口。
4. （备选）使用 **Application** 类型、构建方式选 *Dockerfile*，直接从仓库构建，而不拉取 GHCR 镜像。

> [!NOTE]
> `/shutdown` 端点仅限回环（loopback）客户端访问，无法经由代理触达——所以对外暴露 `/health`、`/v1/models`、`/v1/messages`、`/v1/chat/completions` 是安全的。

### 发布镜像到 GHCR

仓库内置了 GitHub Actions 工作流 [`.github/workflows/docker-publish.yml`](./.github/workflows/docker-publish.yml)，会构建**多架构（amd64 + arm64）**镜像并推送到 GHCR。触发条件：

- 推送到 `main` → 打 `latest` 标签
- 版本 tag `v*` → 语义化标签（如 `v0.2.1` → `0.2.1`、`0.2`）
- 手动 **Run workflow**（`workflow_dispatch`）

它使用内置的 `GITHUB_TOKEN` 鉴权（无需额外 secret）。**首次**成功运行后，到 GitHub 个人主页的 **Packages** 里，如果希望免登录拉取，把该 package 的可见性设为 **Public** 并关联到本仓库即可。
